/*
 * 说明：对应数据库表news_table的DAOImpl类<br>
...
public class NewsTableDAOImpl implements NewsTableDAO {
	private static Logger log = Logger.getLogger(NewsTableDAOImpl.class);
	 ...
	 *            抛出异常
	 */
	private void setFieldFromRs(NewsTable newsTable, ResultSet rs) throws Exception {
		if (rs == null || rs.isAfterLast()) {
			return;
		}
		if (newsTable == null) {
			throw new NullPointerException(
					"NewsTableDAOImpl.java setFieldFromRs() the paramer newsTable is NULL");
		}
		newsTable.setNewsId(rs.getInt("news_id"));
		newsTable.setNewsTitle(rs.getString("news_title"));
		newsTable.setNewsType(rs.getInt("news_type"));
		newsTable.setNewsDate(rs.getString("news_date"));
		newsTable.setIsShow(rs.getString("is_show"));
		newsTable.setIsRoll(rs.getString("is_roll"));
		newsTable.setOrderId(rs.getString("order_id"));
		newsTable.setContent(rs.getString("content"));
		newsTable.setBrowseTimes(rs.getInt("browse_times"));
	}
	 ...
	 * @param newsId
	 *
	 * @return NewsTable对象,可能为null对象
	 */
	public NewsTable selectByPK(int newsId) {
		this.execStatus = 0;
		NewsTable returnValue = null;
		ICon iCon = null;
		try {
			iCon = new ConUtil();
			if (iCon.getCon() == null) {
	...
	public int insert(NewsTable newsTable) {
		...
		try {
			...
			String sql = "INSERT INTO news_table (news_title,news_type,news_date,is_show,is_roll,order_id,content,browse_times) VALUES (?,?,?,?,?,?,?,?)";
			PreparedStatement ps = iCon.prepareStatement(sql);
			if (ps != null) {
				if (newsTable.getNewsTitle() == null) {
					ps.setNull(1, Types.VARCHAR);
				} else {
					ps.setString(1, newsTable.getNewsTitle());
				}
				ps.setInt(2, newsTable.getNewsType());
				if (newsTable.getNewsDate() == null) {
					ps.setNull(3, Types.VARCHAR);
				} else {
					ps.setString(3, newsTable.getNewsDate());
				}
				if (newsTable.getIsShow() == null) {
					ps.setNull(4, Types.CHAR);
				} else {
					ps.setString(4, newsTable.getIsShow());
				}
				if (newsTable.getIsRoll() == null) {
					ps.setNull(5, Types.CHAR);
				} else {
					ps.setString(5, newsTable.getIsRoll());
				}
				if (newsTable.getOrderId() == null) {
					ps.setNull(6, Types.VARCHAR);
				} else {
					ps.setString(6, newsTable.getOrderId());
				}
				if (newsTable.getContent() == null) {
					ps.setNull(7, Types.LONGVARCHAR);
				} else {
					ps.setString(7, newsTable.getContent());
				}
				ps.setInt(8, newsTable.getBrowseTimes());
	...
	public int update(NewsTable newsTable) {
		...
		try {
			...
			String sql = "UPDATE news_table SET news_title=?,news_type=?,news_date=?,is_show=?,is_roll=?,order_id=?,content=?,browse_times=?  WHERE news_id=?";
			PreparedStatement ps = iCon.prepareStatement(sql);
			if (ps != null) {
				if (newsTable.getNewsTitle() == null) {
					ps.setNull(1, Types.VARCHAR);
				...
				ps.setInt(8, newsTable.getBrowseTimes());
				ps.setInt(9, newsTable.getNewsId());
				returnValue = iCon.executePreUpdate();
				if (returnValue > 0) {
					this.execStatus = 1;
				}
	...
	public int updateSelective(NewsTable newsTable) {
		...
		StringBuffer sqlSB = new StringBuffer("UPDATE news_table SET ");
		StringBuffer sqlUpdateSB = new StringBuffer();
		if (newsTable.getNewsTitle() != null) {
			sqlUpdateSB.append(",news_title=?");
			valuesList.add(newsTable.getNewsTitle());
		}
		sqlUpdateSB.append(",news_type=?");
		valuesList.add(newsTable.getNewsType());

		if (newsTable.getNewsDate() != null) {
			sqlUpdateSB.append(",news_date=?");
			valuesList.add(newsTable.getNewsDate());
		}
		if (newsTable.getIsShow() != null) {
			sqlUpdateSB.append(",is_show=?");
			valuesList.add(newsTable.getIsShow());
		}
		if (newsTable.getIsRoll() != null) {
			sqlUpdateSB.append(",is_roll=?");
			valuesList.add(newsTable.getIsRoll());
		}
		if (newsTable.getOrderId() != null) {
			sqlUpdateSB.append(",order_id=?");
			valuesList.add(newsTable.getOrderId());
		}
		if (newsTable.getContent() != null) {
			sqlUpdateSB.append(",content=?");
			valuesList.add(newsTable.getContent());
		}
		sqlUpdateSB.append(",browse_times=?");
		valuesList.add(newsTable.getBrowseTimes());
		...
		sqlSB.append(" WHERE news_id=?");
		valuesList.add(newsTable.getNewsId());
		try {
	...
	public int deleteByPK(int newsId) {
		...
		try {
			...
			String sql = "DELETE FROM news_table  WHERE news_id=?";
			PreparedStatement ps = iCon.prepareStatement(sql);
			if (ps != null) {
				ps.setInt(1,newsId);
	...
	public int delete(NewsTable newsTable) {
		...
		return deleteByPK(newsTable.getNewsId());
	}
	/**
	...
	public int delete(String exeCondition, Object[] values) {
		...
		try {
			...
			String sql = "DELETE FROM news_table ";
			StringBuffer sqlSB = new StringBuffer(sql);
			if (exeCondition == null || "".equals(exeCondition.trim())) {
				return -2;
			} else {
	...
	public long count(String columns, String exeCondition, Object[] values) {
		...
		try {
			...
			StringBuffer sqlSB = new StringBuffer(sql);
			sqlSB.append(columns);
			sqlSB.append(") AS countColumn FROM news_table ");
			if (exeCondition == null || "".equals(exeCondition.trim())) {
				sql = sqlSB.toString();
			} else {
				sqlSB.append(exeCondition);
	...
	public List<NewsTable> getList(String exeCondition, Object[] values, int pageSize, int pageNo) {
		...
		try {
			...
			if (ps != null) {
				...
				NewsTable newsTable;
				if (rs != null && rs.next()) {
					list = new ArrayList<NewsTable>();
					do {
						newsTable = new NewsTable();
						setFieldFromRs(newsTable, rs);
						list.add(newsTable);
					} while (rs.next());
				}
				this.execStatus = 1;
