/*
 * 说明：对应数据库表flash_table的DAOImpl类<br>
...
import hk.com.biosun.dao.FlashTableDAO;
...
public class FlashTableDAOImpl implements FlashTableDAO {
	private static Logger log = Logger.getLogger(FlashTableDAOImpl.class);
	 ...
	 *            表的基础类
	 * @param rs
	 *            数据库表flash_table全列结果集
	 * @throws Exception
	 *            抛出异常
	 */
	private void setFieldFromRs(FlashTable flashTable, ResultSet rs) throws Exception {
		if (rs == null || rs.isAfterLast()) {
			return;
		}
		if (flashTable == null) {
			throw new NullPointerException(
					"FlashTableDAOImpl.java setFieldFromRs() the paramer flashTable is NULL");
		}
		flashTable.setId(rs.getString("id"));
		flashTable.setFlashHeight(rs.getInt("flash_height"));
	}
	...
	public FlashTable select(String exeCondition, Object[] values) {
		...
		try {
			...
			String sql = "SELECT * FROM flash_table ";
	...
	public int insert(FlashTable flashTable) {
		...
		try {
			...
			String sql = "INSERT INTO flash_table (flash_height) VALUES (?)";
			PreparedStatement ps = iCon.prepareStatement(sql);
			if (ps != null) {
				ps.setInt(1, flashTable.getFlashHeight());
	...
	public int update(FlashTable flashTable) {
		...
		try {
			...
			String sql = "UPDATE flash_table SET flash_height=?  WHERE id=?";
			PreparedStatement ps = iCon.prepareStatement(sql);
			if (ps != null) {
				ps.setInt(1, flashTable.getFlashHeight());
				ps.setString(2, flashTable.getId());
				returnValue = iCon.executePreUpdate();
				if (returnValue > 0) {
					this.execStatus = 1;
				}
	...
	public int updateSelective(FlashTable flashTable) {
		...
		StringBuffer sqlSB = new StringBuffer("UPDATE flash_table SET ");
		StringBuffer sqlUpdateSB = new StringBuffer();
		sqlUpdateSB.append(",flash_height=?");
		valuesList.add(flashTable.getFlashHeight());
		...
		valuesList.add(flashTable.getId());
		try {
	 ...
	 *
	 * @param sql  关于flash_table表的SQL语句
	...
	public int deleteByPK(String id) {
		...
		try {
			...
			String sql = "DELETE FROM flash_table  WHERE id=?";
			PreparedStatement ps = iCon.prepareStatement(sql);
			if (ps != null) {
				ps.setString(1,id);
	...
	public int delete(FlashTable flashTable) {
		...
		return deleteByPK(flashTable.getId());
	}
	/**
	...
	public int delete(String exeCondition, Object[] values) {
		...
		try {
			...
			String sql = "DELETE FROM flash_table ";
			StringBuffer sqlSB = new StringBuffer(sql);
			if (exeCondition == null || "".equals(exeCondition.trim())) {
				return -2;
			} else {
	...
	public long count(String columns, String exeCondition, Object[] values) {
		...
		try {
			...
			StringBuffer sqlSB = new StringBuffer(sql);
			sqlSB.append(columns);
			sqlSB.append(") AS countColumn FROM flash_table ");
			if (exeCondition == null || "".equals(exeCondition.trim())) {
				sql = sqlSB.toString();
			} else {
				sqlSB.append(exeCondition);
	...
	public List<FlashTable> getList(String exeCondition, Object[] values, int pageSize, int pageNo) {
		...
		try {
			...
			if (ps != null) {
				...
				if (rs != null && rs.next()) {
					list = new ArrayList<FlashTable>();
					do {
						flashTable = new FlashTable();
						setFieldFromRs(flashTable, rs);
						list.add(flashTable);
					} while (rs.next());
				}
				this.execStatus = 1;
