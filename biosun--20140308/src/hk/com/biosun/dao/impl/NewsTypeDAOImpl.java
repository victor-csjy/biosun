/*
 * 说明：对应数据库表news_type的DAOImpl类<br>
...
public class NewsTypeDAOImpl implements NewsTypeDAO {
	private static Logger log = Logger.getLogger(NewsTypeDAOImpl.class);
	 ...
	 *            抛出异常
	 */
	private void setFieldFromRs(NewsType newsType, ResultSet rs) throws Exception {
		if (rs == null || rs.isAfterLast()) {
			return;
		}
		if (newsType == null) {
			throw new NullPointerException(
					"NewsTypeDAOImpl.java setFieldFromRs() the paramer newsType is NULL");
		}
		newsType.setNewsType(rs.getInt("news_type"));
		newsType.setOrderId(rs.getString("order_id"));
		newsType.setNewsName(rs.getString("news_name"));
	}
	 ...
	 * @param newsType
	 *
	 * @return NewsType对象,可能为null对象
	 */
	public NewsType selectByPK(int newsType) {
		this.execStatus = 0;
		NewsType returnValue = null;
		ICon iCon = null;
		try {
			iCon = new ConUtil();
			if (iCon.getCon() == null) {
	...
	public int insert(NewsType newsType) {
		...
		try {
			...
			String sql = "INSERT INTO news_type (news_type,order_id,news_name) VALUES (?,?,?)";
			PreparedStatement ps = iCon.prepareStatement(sql);
			if (ps != null) {
				ps.setInt(1, newsType.getNewsType());
				if (newsType.getOrderId() == null) {
					ps.setNull(2, Types.VARCHAR);
				} else {
					ps.setString(2, newsType.getOrderId());
				}
				if (newsType.getNewsName() == null) {
					ps.setNull(3, Types.VARCHAR);
				} else {
					ps.setString(3, newsType.getNewsName());
				}
	...
	public int update(NewsType newsType) {
		...
		try {
			...
			String sql = "UPDATE news_type SET order_id=?,news_name=?  WHERE news_type=?";
			PreparedStatement ps = iCon.prepareStatement(sql);
			if (ps != null) {
				if (newsType.getOrderId() == null) {
					ps.setNull(1, Types.VARCHAR);
				} else {
					ps.setString(1, newsType.getOrderId());
				}
				if (newsType.getNewsName() == null) {
					ps.setNull(2, Types.VARCHAR);
				} else {
					ps.setString(2, newsType.getNewsName());
				}
				ps.setInt(3, newsType.getNewsType());
				returnValue = iCon.executePreUpdate();
				if (returnValue > 0) {
					this.execStatus = 1;
				}
	...
	public int updateSelective(NewsType newsType) {
		...
		StringBuffer sqlSB = new StringBuffer("UPDATE news_type SET ");
		StringBuffer sqlUpdateSB = new StringBuffer();
		if (newsType.getOrderId() != null) {
			sqlUpdateSB.append(",order_id=?");
			valuesList.add(newsType.getOrderId());
		}
		if (newsType.getNewsName() != null) {
			sqlUpdateSB.append(",news_name=?");
			valuesList.add(newsType.getNewsName());
		}
		...
		sqlSB.append(" WHERE news_type=?");
		valuesList.add(newsType.getNewsType());
		try {
	...
	public int deleteByPK(int newsType) {
		...
		try {
			...
			String sql = "DELETE FROM news_type  WHERE news_type=?";
			PreparedStatement ps = iCon.prepareStatement(sql);
			if (ps != null) {
				ps.setInt(1,newsType);
	...
	public int delete(NewsType newsType) {
		...
		return deleteByPK(newsType.getNewsType());
	}
	/**
	...
	public int delete(String exeCondition, Object[] values) {
		...
		try {
			...
			String sql = "DELETE FROM news_type ";
			StringBuffer sqlSB = new StringBuffer(sql);
			if (exeCondition == null || "".equals(exeCondition.trim())) {
				return -2;
			} else {
	...
	public long count(String columns, String exeCondition, Object[] values) {
		...
		try {
			...
			StringBuffer sqlSB = new StringBuffer(sql);
			sqlSB.append(columns);
			sqlSB.append(") AS countColumn FROM news_type ");
			if (exeCondition == null || "".equals(exeCondition.trim())) {
				sql = sqlSB.toString();
			} else {
				sqlSB.append(exeCondition);
